...
def make_file():
    pcos = build_precinct_dict('munger/pco37_new.csv')
    path = 'munger/votdst_area.kml'
    tree = ET.parse(path)
    folder = tree.getroot()[0][1]
    all_precincts = {"type": "FeatureCollection", "features" : []}
    features = all_precincts["features"]
    for placemark in folder:
        if placemark.tag != '{http://www.opengis.net/kml/2.2}Placemark':
            continue
        name = placemark[0].text
        if name.startswith('SEA 37-') or name.startswith('REN 37-'):
            # style = placement[1]
            ...
            with open('precincts/chalicelib/' + str(ident) + '.json', 'w') as outfile:
                features.append(precinct)
                json.dump(precinct, outfile, indent=2)

    with open('precincts/chalicelib/' 'all_precincts.json', 'w') as outfile:
        json.dump(all_precincts, outfile, indent=2)         
make_file()
